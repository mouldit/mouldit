...
<h2>Principle</h2>
<p>Via the Mouldit CLI you can configure your crud actions as well as the entire frontend of your application. The CLI will infer all possible crudactions from your schema, or as for now at least all implemented actions (see list below). The configuration of such an action is explained in detail below. After all actions are configured - which will result in Mouldit generating all necessary rest API's of your application - the Mouldit CLI will guide you through the configuration of the frontend. This configuration exists out of two parts. First you specify general stuff about this frontend, like navigation and pages. Then you will configure all UI components and their behaviour, which include triggering the above mentioned crudactions and dealing with their results.</p>
